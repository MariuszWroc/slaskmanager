...
-- -------------------------------------------------------
CREATE TABLE IF NOT EXISTS `slaskdb`.`user_role`(
    `id` INT(11) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `slaskdb`.`user_data`(
	...
	PRIMARY KEY (`id`),
            ...
            CONSTRAINT `fk_user_data_role`
              ...
              REFERENCES `slaskdb`.`user_role` (`id`)
              ON DELETE NO ACTION
              ON UPDATE NO ACTION)
          ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Data for table `slaskdb`.`user_role`
-- -----------------------------------------------------
START TRANSACTION;
USE `slaskdb`;
INSERT INTO `slaskdb`.`user_role` (`id`, `role_name`) VALUES (1, 'ROLE_ADMIN');
INSERT INTO `slaskdb`.`user_role` (`id`, `role_name`) VALUES (2, 'ROLE_USER');
INSERT INTO `slaskdb`.`user_role` (`id`, `role_name`) VALUES (3, 'ROLE_CODER');
...
START TRANSACTION;
USE `slaskdb`;
INSERT INTO `slaskdb`.`game` (`id`, `game_code`, `session_id`, `game_name`, `user_status`, `next_turn_wanted`, `club_idClub`, `user_data_idUser`) VALUES (1, '1', '1', 'mariuszGra', 1, 0, 1, 1);
INSERT INTO `slaskdb`.`game` (`id`, `game_code`, `session_id`, `game_name`, `user_status`, `next_turn_wanted`, `club_idClub`, `user_data_idUser`) VALUES (2, '1', '2', 'jacekGra', 0, 0, 2, 2);
