...
CREATE TABLE IF NOT EXISTS `databaseInstance`.`user_data`(
	...
	PRIMARY KEY (`id`),
            ...
            CONSTRAINT `fk_user_data_role`
              ...
              REFERENCES `databaseInstance`.`role` (`id`)
              ON DELETE NO ACTION
              ON UPDATE NO ACTION)
          ENGINE = InnoDB;
...
START TRANSACTION;
USE `databaseInstance`;
INSERT INTO `databaseInstance`.`role` (`id`, `role_name`) VALUES (1, 'ROLE_ADMIN');
INSERT INTO `databaseInstance`.`role` (`id`, `role_name`) VALUES (2, 'ROLE_USER');
INSERT INTO `databaseInstance`.`role` (`id`, `role_name`) VALUES (3, 'ROLE_CODER');
